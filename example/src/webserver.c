 ...
 ****************************************************************************/
const char* indexName = "/page2.htm";
const char* ToggleLEDHandler(int iIndex, int iNumParams, char * pcParam[], char * pcValue[]);
const tCGI CGI_URI_MAP = {"/toggle_led.cgi",ToggleLEDHandler};
uint32_t g_LED_Status = -1;
 char g_F_Name[15];
...
const char* ToggleLEDHandler(int iIndex, int iNumParams, char * pcParam[], char * pcValue[]){
	...
	for(i=0;i<iNumParams;i++){
		...
		if(strncmp(pcParam[i],"LED",9)==0){
			DEBUGOUT(pcParam[i]);
					DEBUGOUT(pcValue[i]);
					DEBUGOUT("\n");
			if(strncmp(pcValue[i],"blue",4)==0){
				g_LED_Status = 1;
			}
			else if(strncmp(pcValue[i],"green",5)==0){
				g_LED_Status = 2;
			}
			else if(strncmp(pcValue[i],"red",3)==0){
				g_LED_Status = 3;
			}
		}

		else if(strncmp(pcParam[i],"string",6)==0){
				DEBUGOUT(pcParam[i]);
				DEBUGOUT(pcValue[i]);
				DEBUGOUT("\n");

				strcpy(g_F_Name,pcValue[i]);
				DEBUGOUT(g_F_Name);
				/*
				g_str = (char *)malloc(sizeof(pcValue[i])*sizeof(char)+2);
			...
			/*}
				...
				DEBUGOUT("ERROR: Memory Allocation failed.\n");
			}*/
		}
 	}
	return indexName;
...
int main(void)
{
	...
	http_set_cgi_handlers(&CGI_URI_MAP, 1);
	http_set_ssi_handler(SSI_Pot_Handler,(char const**)TAGS,1);
	...
	while (1) {
		...
		if(g_str_f == 1){
			...
			LCD_PutStrXY(13, 25,"              ");
			LCD_PutStrXY(13, 25,g_F_Name);
			g_str_f = 0;
		}
		/*
		if(g_str_f == 1 && strlen(g_str)<15){
			//LCD_PutStrXY(13,25,"");
			DEBUGOUT("\nThis string is from main:  ->");
			DEBUGOUT(g_str);
			LCD_PutStrXY(13, 25,g_str);
			g_str_f = 0;
			...
			DEBUGOUT("Len is : %d\n",strlen(g_str));
			//DEBUGOUT(g_str);
			DEBUGOUT("String too long, try smaller\n");
			g_str_f = 0;
		}
		 */
		if(g_LED_Status != -1){
			...
			else if(g_LED_Status == 2){
				Board_LED_Set(1,true);
			}
			else if(g_LED_Status == 3){
